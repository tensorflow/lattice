...
class ToolsTestCase(test.TestCase):
  ...
  def testCastToDictOfTensorScalars(self):
    ...
    self.assertIsInstance(got['a'], ops.Tensor)
...
class LatticeToolsTestCase(test.TestCase):

  def setUp(self):
    super(LatticeToolsTestCase, self).setUp()
...
class Lattice1DSliceTestCase(test.TestCase):

  def setUp(self):
    super(Lattice1DSliceTestCase, self).setUp()
  ...
  def testTwodLatticeSlice(self):
    ...
    # param[0][2] = 4
    # param[1][2] = 5
    lattice_param_tensor = array_ops.constant([list(range(2 * 3))])
    # param[0][:] = [0, 2, 4]
  ...
  def testThreedLatticeSlice(self):
    ...
    # param[0][2][1] = 10
    # param[1][2][1] = 11
    lattice_param_tensor = array_ops.constant([list(range(2 * 3 * 2))])
  ...
  def testWrongTensorShapeExpectsError(self):
    lattice_param_tensor = array_ops.placeholder(
        shape=(2, 2, 2), dtype=dtypes.float32)
    with self.assertRaises(ValueError):
