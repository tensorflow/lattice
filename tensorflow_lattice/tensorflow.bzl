...
# ==============================================================================
"""This is a modified copy of github.com/tensorflow/tensorflow/tensorflow.bzl.
...
Changes include
1. copied and pasted "if_static" from build_config_root.bzl
2. copied and pasted "if_mkl" from tensorflow/third_party/mkl/build_defs.bzl
3. add tensorflow root to rpath (see _add_tf_search_path, and
   _make_search_paths)
4. Replace //tensorflow with @org_tensorflow//tensorflow.
5. Replace //third_party with @org_tensorflow//third_party.
"""
# -*- Python -*-
...
def if_static(extra_deps, otherwise=[]):
  return select({
      "@org_tensorflow//tensorflow:framework_shared_object": otherwise,
...
def if_mkl(if_true, if_false = []):
    ...
    """
    return select({
        "@org_tensorflow//third_party/mkl:using_mkl": if_true,
...
def tf_android_core_proto_sources(core_proto_sources_relative):
  return [
      "@org_tensorflow//tensorflow/core:" + p for p in core_proto_sources_relative
  ]
...
def tf_android_core_proto_headers(core_proto_sources_relative):
  return ([
      "@org_tensorflow//tensorflow/core/" + p.replace(".proto", ".pb.h")
...
def if_not_windows(a):
  return select({
      ...
      clean_dep("@org_tensorflow//tensorflow:windows_msvc"): [],
      "//conditions:default": a,
  })
...
def if_darwin(a):
  return select({
      clean_dep("@org_tensorflow//tensorflow:darwin"): a,
      "//conditions:default": [],
  })
...
def tf_copts():
  return (if_not_windows([
      ...
      ["-mfpu=neon"]) + if_linux_x86_64(["-msse3"]) + select({
          ...
          clean_dep("@org_tensorflow//tensorflow:windows"): WIN_COPTS,
          clean_dep("@org_tensorflow//tensorflow:windows_msvc"): WIN_COPTS,
          clean_dep("@org_tensorflow//tensorflow:ios"): ["-std=c++11"],
...
def _make_search_paths(prefix, levels_to_root, add_tf=False):
  paths = ",".join(
...
# to define symbols from @org_tensorflow//tensorflow/core:framework and @org_tensorflow//tensorflow/core:lib.
...
# (@org_tensorflow//third_party/tensorflow:libtensorflow_framework.so) when not building
...
def tf_gen_op_wrappers_cc(name,
  ...
  native.cc_library(
      ...
      deps=deps + if_not_android([
          ...
          clean_dep("@org_tensorflow//tensorflow/core:android_tensorflow_lib"),
      ]),
      copts=tf_copts(),
      alwayslink=1,
...
def tf_gen_op_wrapper_py(name,
  ...
  tf_cc_binary(
      ...
      deps=([
          clean_dep("@org_tensorflow//tensorflow/core:framework"),
          clean_dep("@org_tensorflow//tensorflow/python:python_op_gen_main")
      ] + deps),
      visibility=[clean_dep("@org_tensorflow//tensorflow:internal")],)
  ...
  native.py_library(
      ...
      visibility=visibility,
      deps=[
          clean_dep("@org_tensorflow//tensorflow/python:framework_for_generated_wrappers_v2"),
      ],)
...
def tf_cc_test(name,
  ...
  native.cc_test(
      ...
      linkstatic=linkstatic or select({
          ...
          clean_dep("@org_tensorflow//tensorflow:darwin"): 1,
...
def tf_cuda_only_cc_test(name,
  ...
  native.cc_test(
      ...
      data=data,
      deps=deps + if_cuda([
          clean_dep("@org_tensorflow//tensorflow/core:cuda"),
...
def tf_py_wrap_cc(name,
  ...
  extra_linkopts = select({
      "@local_config_cuda//cuda:darwin": [
          ...
          clean_dep("@org_tensorflow//tensorflow:tf_exported_symbols.lds")
      ],
      clean_dep("@org_tensorflow//tensorflow:windows"): [],
      clean_dep("@org_tensorflow//tensorflow:windows_msvc"): [],
      "//conditions:default": [
          "-Wl,--version-script",
          clean_dep("@org_tensorflow//tensorflow:tf_version_script.lds")
      ]
  })
  extra_deps += select({
      "@local_config_cuda//cuda:darwin": [
  ...
  native.py_library(
      ...
      srcs_version="PY2AND3",
      data=select({
          clean_dep("@org_tensorflow//tensorflow:windows"): [":" + cc_library_pyd_name],
...
def tf_generate_proto_text_sources(name, srcs_relative_dir, srcs):
  ...
  native.genrule(
      name=name,
      srcs=srcs + [clean_dep("@org_tensorflow//tensorflow/tools/proto_text:placeholder.txt")],
...
def tf_version_info_genrule():
  native.genrule(
      ...
      srcs=[
          clean_dep("@org_tensorflow//tensorflow/tools/git:gen/spec.json"),
          clean_dep("@org_tensorflow//tensorflow/tools/git:gen/head"),
          clean_dep("@org_tensorflow//tensorflow/tools/git:gen/branch_ref"),
      ],
      outs=["util/version_info.cc"],
      cmd=
      "$(location @org_tensorflow//tensorflow/tools/git:gen_git_source.py) --generate $(SRCS) \"$@\"",
      local=1,
      tools=[clean_dep("@org_tensorflow//tensorflow/tools/git:gen_git_source.py")],)
...
def tf_py_build_info_genrule():
  native.genrule(
      ...
      cmd=
      "$(location @org_tensorflow//tensorflow/tools/build_info:gen_build_info.py) --raw_generate \"$@\" --build_config " + if_cuda("cuda", "cpu"),
      local=1,
      tools=[clean_dep("@org_tensorflow//tensorflow/tools/build_info:gen_build_info.py")],)
